...
import (
	...
	//RPCConfig is in ./config/config.go
	...
	//NodeInfo is in ./p2p/node_info.go
	//IPeerSet is in ./p2p/peer_set.go
	"github.com/YunweiMao/tendermint/p2p"

	//AppConnQuery is in ./proxy/app_conn.go
	//AppConnMempool is in ./proxy/app_conn.go
	"github.com/YunweiMao/tendermint/proxy"

	//Store is in ./state/store.go
	//BlockStore is in ./state/services.go
	//EvidencePool is in ./state/services.go
	sm "github.com/YunweiMao/tendermint/state"

	//BlockIndexer is in ./state/indexer/block.go
	"github.com/YunweiMao/tendermint/state/indexer"

	//TxIndexer is in ./state/txindex/indexer.go
	"github.com/YunweiMao/tendermint/state/txindex"

	//Reactor is in ./consensus/reactor.go
	"github.com/YunweiMao/tendermint/consensus"

	//Mempool is in ./mempool/mempool.go
